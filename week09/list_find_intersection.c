// COMP1511 24T1 Tutorial 09 - list_find_intersection.c
//
// Given two linked lists, return a new list that is
// constructed of nodes containing any values that appear
// in both lists.
...
struct node *list_find_intersection(
    struct node *head1,
    struct node *head2
) {
    ...
    while (head1 != NULL) {
        if (
            exists_in_list(head2, head1->data) &&
